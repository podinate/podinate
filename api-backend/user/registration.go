...
func RegisterUser(user goth.User) (*User, error) {
	...
	if err != nil {
		eh.Log.Infow("User not found, inserting new user", "provider", user.Provider, "provider_id", user.UserID)
		...
		eh.Log.Infow("user registered", "user_uuid", authorisedUser, "user", out)
		return out, err
	}
	...
	if err != nil {
		eh.Log.Errorw("Error updating oauth_login", "error", err)
	...
	if err != nil {
		eh.Log.Errorw("Error updating user", "error", err, "user_uuid", authorisedUser)
		return &User{}, errors.New("Error updating user")
	}
	out, err := GetByUUID(authorisedUser)
	eh.Log.Infow("User updated", "reason", "new login, updated details from oauth", "user_uuid", authorisedUser, "user", user)
	return out, err
}
